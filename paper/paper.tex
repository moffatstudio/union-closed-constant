...
In particular, for $w=0.810222$ and $c=0.38284$, taking $\eta=10^{-4}$ gives a ratio of at least $1.048$ whenever $\E_\mu\ent(X)\le10^{-7}$, and letting $\eta\to0$ gives $\liminf\ge2w(1-c)$ along any sequence with $\E_\mu\ent(X)\to0$.
\end{lemma}
\begin{proof}
...
and $a\ge1-c-\eta-m\ge1-c-\eta-\E_\mu\ent(X)/\ent(\eta)$. The numerical instance is $2\cdot0.810222\cdot\kappa(10^{-4})\cdot(1-0.38284-10^{-4}-10^{-7}/\ent(10^{-4}))=1.0481$.
\end{proof}
...
For $w=0.810222$ and $c=0.38284$, the infimum of $R$ over $q\in[0,1]$ and laws $P_0,P_1$ with at most four atoms each, $\E_\mu[X]\ge1-c$ and $\int\ent\,d\mu\ge10^{-7}$, is $1.0000733$, attained at the two-point law of Table~\ref{tab:cert}.
\end{hypothesis}
Hypothesis~\ref{hyp:global} is supported by Table~\ref{tab:cert} and by the same minima with the floor lowered to $10^{-7}$ (in every case the same minimiser and the same value). The complementary region $\int\ent\,d\mu<10^{-7}$ is covered unconditionally by Lemma~\ref{lem:small}. The hypothesis plays the role of the ``global minimiser structure'' hypothesis in \cite[Theorem~13]{Liu}.
...
\end{theorem}
\begin{proof}
Let $\mu$ have $\E_\mu[X]\ge1-c$ and let $P\in\cC_3(\mu)$. By Liu's Theorem~9 we may assume $P$ is of the form \eqref{eq:twoprod}. If $\int\ent\,d\mu=0$ both sides of \eqref{eq:cert} vanish. If $0<\int\ent\,d\mu<10^{-7}$, Lemma~\ref{lem:small} gives $R\ge1.048>C$. Otherwise, by Proposition~\ref{prop:atoms} (with $C=1.00005$), $\Phi_C(q,P_0,P_1)\ge\Phi_C(q,P_0',P_1')$ for some pair $(P_0',P_1')$ with at most four atoms each on the same slice (so with the same $\E_\mu[X]$). If $\int\ent\,d\mu'\ge10^{-7}$, Hypothesis~\ref{hyp:global} gives $R\ge1.0000733>C$, so $\Phi_C(q,P_0',P_1')\ge0$; if $0<\int\ent\,d\mu'<10^{-7}$, Lemma~\ref{lem:small} gives $R\ge1.048>C$ and again $\Phi_C\ge0$; if $\int\ent\,d\mu'=0$ then $\Phi_C(q,P_0',P_1')\ge0$ trivially. Proposition~\ref{prop:cert} concludes. The final sentence follows from Proposition~\ref{prop:twoadv}(c) and the same computation.
\end{proof}
