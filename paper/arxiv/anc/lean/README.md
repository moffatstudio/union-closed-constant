...
* **Protocols by their `(0,0)`-value.** A protocol is modelled by `Π : ℝ → ℝ → ℝ`,
  ...
  facts it does use (`Π x 0 = 0`, `Π 1 1 = 1`, `Π 1 y = y`) are derived from these bounds;
  `ent(Π) ≤ 1` (`h_le_one`) holds for every real argument, from Mathlib's
...
`c = 0.3829` is `≈ 1.5·10⁻⁵ (exactly 1.474·10⁻⁵)` with the entropy bounds above). Since
